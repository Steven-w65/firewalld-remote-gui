...
28. [ ] Display runtime and permanent zone information, including activity, interfaces, sources, services, ports, protocols, masquerade, forwarding, and rich-rule count.
   ...
   - [ ] **ADAPTER-ONLY — no live-host mutation:** Simulate a partial Runtime + Permanent result only with the offline fake adapter; capture the separate success/failure phases, refreshed state, no rollback, no retry, and no automatic reload. Do not point this simulation at a remote host.
